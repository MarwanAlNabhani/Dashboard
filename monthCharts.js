...
regionApiUrl = monthlyRegionURL(currentValue);
statusApiUrl = monthlyStatusURL(currentValue);
queueApiUrl = monthlyQueueURL(currentValue);
issueCategoryApiUrl = monthlyIssueCategoryURL(currentValue);
...
fetch(regionApiUrl)
    ...
    const range = calculateMonthsAgoRange(currentValue); // calculate range for 6 months ago
...
function monthlyRegionURL(month) {
    return `http://localhost:5500/monthly-data/${month}/REGION`;
}
function monthlyStatusURL(month) {
    return `http://localhost:5500/monthly-data/${month}/STATUS`;
}
function monthlyQueueURL(month) {
    return `http://localhost:5500/monthly-data/${month}/QUEUE`;
}
function monthlyIssueCategoryURL(month) {
    return `http://localhost:5500/monthly-data/${month}/ISSUECATEGORY`;
}
...
function calculateMonthsAgoRange(numMonthsAgo) {
    const today = new Date();
    const start = new Date(today.getFullYear(), today.getMonth() - numMonthsAgo, 1);
    const end = new Date(today.getFullYear(), today.getMonth() - numMonthsAgo + 1, 0);
    return { start, end };
}
